param(
    ...
    [string]$Node2BaseUrl = 'http://localhost:8090',
...
Write-Host "Posting signed alert to redis node 1..."
...
Write-Host "Inspecting replay-store from redis node 2..."
...
Write-Host "Replaying the same signed alert against redis node 2. Expecting replay protection..."
try {
...
Invoke-RestMethod -Method Get -Uri "$Node1BaseUrl/demo-tenants/observability" -Headers $adminHeaders | ConvertTo-Json -Depth 6
Invoke-RestMethod -Method Get -Uri "$Node2BaseUrl/demo-tenants/observability" -Headers $adminHeaders | ConvertTo-Json -Depth 6
