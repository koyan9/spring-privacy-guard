...
NODE2_BASE_URL="http://localhost:8092"
...
echo "Posting signed alert to custom JDBC node 1..."
curl -fsSL "${receiver_headers[@]}" -H "Content-Type: application/json" -d "$payload" "$NODE1_BASE_URL/demo-alert-receiver" | jq .

echo "Checking replay-store visibility from custom JDBC node 2..."
curl -fsSL "${admin_headers[@]}" "$NODE2_BASE_URL/demo-alert-receiver/replay-store?limit=20&offset=0" | jq .

echo "Replaying the same signed alert against custom JDBC node 2. Expecting replay protection..."
status=$(
  curl -sS -o /dev/null \
    "${receiver_headers[@]}" \
...
node1_audit_impl=$(echo "$node1_observability" | jq -r '.repositoryImplementations.audit')
node2_audit_impl=$(echo "$node2_observability" | jq -r '.repositoryImplementations.audit')
if [[ "$node1_audit_impl" != "CustomJdbcTenantAuditRepository" || "$node2_audit_impl" != "CustomJdbcTenantAuditRepository" ]]; then
  echo "Expected both nodes to use CustomJdbcTenantAuditRepository" >&2
  exit 1
fi

node1_dead_letter_impl=$(echo "$node1_observability" | jq -r '.repositoryImplementations.deadLetter')
node2_dead_letter_impl=$(echo "$node2_observability" | jq -r '.repositoryImplementations.deadLetter')
if [[ "$node1_dead_letter_impl" != "CustomJdbcTenantDeadLetterRepository" || "$node2_dead_letter_impl" != "CustomJdbcTenantDeadLetterRepository" ]]; then
  echo "Expected both nodes to use CustomJdbcTenantDeadLetterRepository" >&2
  exit 1
fi
...
if [[ "$node1_backend" != "JDBC" || "$node2_backend" != "JDBC" ]]; then
  echo "Expected both nodes to expose receiverReplayStore.backend=JDBC" >&2
  exit 1
fi
