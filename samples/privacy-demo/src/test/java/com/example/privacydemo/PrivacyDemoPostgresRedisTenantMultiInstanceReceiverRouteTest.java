...
class PrivacyDemoPostgresRedisTenantMultiInstanceReceiverRouteTest extends SampleMultiInstanceHttpTestSupport {

    @TempDir
    Path tempDir;

    @BeforeEach
    void resetSharedReplayStore() {
        SharedReplayStoreConfig.REPLAY_STORE.clear();
    }

    @Test
    void rejectsReplayAcrossNode1AndNode2ForTenantSpecificRouteUsingSharedReplayStoreUnderProductionLikeProfile() throws Exception {
        String jdbcUrl = buildFileBackedH2JdbcUrl(tempDir.resolve("privacy-demo-postgres-redis-route-shared"));

        try (ConfigurableApplicationContext node1 = startNode(
                jdbcUrl,
                new Class[]{PrivacyGuardDemoApplication.class, SharedReplayStoreConfig.class},
                new String[]{"postgres-redis-tenant"},
                "--server.port=0",
             ...
             ConfigurableApplicationContext node2 = startNode(
                     jdbcUrl,
                     new Class[]{PrivacyGuardDemoApplication.class, SharedReplayStoreConfig.class},
                     new String[]{"postgres-redis-tenant", "postgres-redis-tenant-node2"},
                     "--server.port=0",
                     "--spring.datasource.url=" + jdbcUrl,
                     "--spring.datasource.driver-class-name=org.h2.Driver",
                     "--spring.datasource.username=sa",
            ...
            assertThat(getJsonObject(client, node1Port, "/demo-tenants/current", Map.of()).get("instanceId")).isEqualTo("postgres-redis-node-1");
            assertThat(getJsonObject(client, node2Port, "/demo-tenants/current", Map.of()).get("instanceId")).isEqualTo("postgres-redis-node-2");
            ...
            String nonce = "multi-node-postgres-tenant-a-route-" + java.util.UUID.randomUUID();
    ...
    static class SharedReplayStoreConfig {

        private static final InMemoryPrivacyAuditDeadLetterWebhookReplayStore REPLAY_STORE =
                new InMemoryPrivacyAuditDeadLetterWebhookReplayStore();
